            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (value === '' || /^\d+$/.test(value)) {
                  onFormChange({ discountValue: value === '' ? 0 : parseInt(value) });
                }
              }}
              onBlur={(e) => {
                ...
                if (couponForm.discountType === 'percentage') {
                  if (value > MAX_DISCOUNT_PERCENTAGE) {
                    ...
                    onFormChange({ discountValue: MAX_DISCOUNT_PERCENTAGE });
                  } else if (value < 0) {
                    onFormChange({ discountValue: 0 });
                  }
                } else {
                  if (value > MAX_DISCOUNT_AMOUNT) {
                    addNotification(`할인 금액은 ${MAX_DISCOUNT_AMOUNT.toLocaleString()}원을 초과할 수 없습니다`, 'error');
                    onFormChange({ discountValue: MAX_DISCOUNT_AMOUNT });
                  } else if (value < 0) {
                    onFormChange({ discountValue: 0 });
                  }
                }
