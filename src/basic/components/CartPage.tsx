...
function CartPage() {
  ...
  }, [cart, addToCart, getRemainingStock, addNotification]);

  const handleApplyCoupon = useCallback((coupon: typeof coupons[0]) => {
  ...
  }, [applyCoupon, calculateTotal, addNotification]);
            ...
            <section className="bg-white rounded-lg border border-gray-200 p-4">
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-3">
                  {cart.map(item => {
                    const itemCart = cart; // 전체 cart를 전달하기 위해
                    const itemDiscount = item.product.discounts.reduce((max, d) => {
                      return item.quantity >= d.quantity && d.rate > max ? d.rate : max;
                    }, 0);
                    const hasBulk = cart.some(c => c.quantity >= 10);
                    const finalDiscount = hasBulk ? Math.min(itemDiscount + 0.05, 0.5) : itemDiscount;
                    const itemTotal = Math.round(item.product.price * item.quantity * (1 - finalDiscount));
