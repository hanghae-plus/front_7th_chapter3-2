...
import { Notification, Toast } from "./components/ui/Toast";
...
const App = () => {
  ...
  const formatPriceLegacy = (price: number, productId?: string): string => {
    if (productId) {
  ...
  const formattedPrice = (price: number, productId?: string): string => {
    if (productId) {
      const product = products.find((p) => p.id === productId);
      if (product && getRemainingStock(product) <= 0) {
        return "SOLD OUT";
    ...
    return formatPrice(price, { useSymbol: !isAdmin });
  };

  // const price = formattedPrice(price, productId);
